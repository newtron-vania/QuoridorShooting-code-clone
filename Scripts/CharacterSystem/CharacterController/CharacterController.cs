...
public partial class CharacterController : MonoBehaviour, IEventListener
{
    // -----------------------------
    // 프리팹 관련 변수
    // -----------------------------
    ...
    public BaseCharacter SaveTargetBaseCharacter // 공격관련 하이라이팅 및 타겟 정보 저장
    {
        ...
        set
        {
            ...
            if (_saveTargetBaseCharacter == null)
            {
                ...
                UIManager.Instance.MakeHighlighting<TokenHighLighting>(GameManager.Instance.CharacterController.GetObjectToPosition(GameManager.ToWorldPosition(value.Position)).transform);
    ...
    public bool IsConfirm // 확인버튼 활성화 여부 저장
    {
        get
        {
            return _saveCurrentTargetPoint || _saveTargetBaseCharacter != null || SaveCurrentAbilityPoint || (_previewWall != null && _previewWall.gameObject.activeInHierarchy);
        }
    }
    ...
    private void OnChangeCharacterState(PlayerControlStatus beforeState, PlayerControlStatus currentState)
    {
        ...
        if (beforeState == PlayerControlStatus.Build)
        {
            ...
            foreach (Transform child in GameManager.Instance.WallHighlightingParent.transform)
            {
        ...
        if (TouchType == TouchUtils.TouchType.UseSupply)
        {
            if (TouchState == TouchUtils.TouchState.Began)
            {
                ...
                if (hit.collider)
                {
                    ...
                    {
                        case SupplymentData.SupplyTarget.TargetEnemy:
                            ...
                            break;
                        case SupplymentData.SupplyTarget.TargetChar:
                ...
                else
                {
                    ...
                    SupplyManager.Instance._supplyTarget = SupplymentData.SupplyTarget.None;
                    TouchType = TouchUtils.TouchType.CharacterSelect;
